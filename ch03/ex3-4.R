...
# 종교별 전체 교인의 수 차트
q1 = ggplot(aes(x=religion, y=total.count), data=pew.agg,         # 차트의 XY축과 데이터 및 형태를 지정한다.
            width=0.5) + geom_bar(stat="identity") + coord_flip() # 차트의 세부 모양과 방향을 지정한다.

# 종교별 평균 소득 플롯
q2 = ggplot(aes(x=religion, y=avg.income.usd), data=pew.agg,
           width=0.5) + geom_bar(stat="identity") + coord_flip()
